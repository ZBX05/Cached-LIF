...
def main():
    ...
    parser.add_argument('--weight_decay',type=float,default=4e-5,help='Weight decay. Default: 4e-5.')
    ...
    parser.add_argument('--trtloss',type=int,default=0,help='Whether to use the Temporal Regularization Training method. Default: True.')
    ...
    parser.add_argument('--topology',type=str,default='ResNet-19',help='Choice of network topology. Format for convolutional layers: CONV_{output channels}-{kernel size}-{stride}-{padding}. Format for fully-connected layers: FC-{output units}.')
    parser.add_argument('--norm',type=str,choices=['BN','No'],default='BN',help='Choice of normalization method - batch normalization (BN), temporal decoupled batch normalization (tdBN). Default: BN.')
    ...
    parser.add_argument('--parameterize',type=int,default=0)
    parser.add_argument('--alpha',type=float,default=0.2,help='Default: 0.5.')
    parser.add_argument('--beta',type=float,default=-0.5,help='Default: 0.5.')
    ...
    model=SNN(args.topology,args.T,input_shape,args.dropout,args.norm,args.v_threshold,args.v_reset,args.tau,args.alpha,args.beta,
              args.surrogate_type,surrogate_param,args.expend_time,args.init,args.parameterize)
    if args.parallel:
