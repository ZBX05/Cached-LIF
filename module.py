...
class FCBlock(BaseBlock):
    def __init__(self,input_size:int,output_size:int,bias:bool,dropout:float,v_threshold:float=1.0,v_reset:float=0.0,tau:float=5,
                 alpha:float=0.1,beta:float=0.5,surrogate_type:str='sigmoid',surrogate_param:float=2.0) -> None:
        super(FCBlock,self).__init__()
        self.fc=SeqToANNContainer(
            nn.Linear(input_size,output_size,bias=bias)
        )
        self.lif=LIFNode(v_threshold=v_threshold,v_reset=v_reset,tau=tau,alpha=alpha,beta=beta,surrogate_type=surrogate_type,surrogate_param=surrogate_param)
        if dropout!=0:
...
class Conv2dAPBlock(Conv2dBlock):
    def __init__(self,norm:str,input_shape:tuple,in_channels:int,out_channels:int,conv_kernel_size:int,conv_stride:int,conv_padding:int,
        ...
        super(Conv2dAPBlock,self).__init__(norm,input_shape,in_channels,out_channels,conv_kernel_size,conv_stride,conv_padding,
                                                  pool_kernel_size,pool_stride,pool_padding,bias,v_threshold,v_reset,tau,alpha,beta,surrogate_type,
                                                  surrogate_param)
        self.maxpool2d_layer=SeqToANNContainer(
...
class Conv2dNoPoolingBlock(Conv2dBlock):
    def __init__(self,norm:str,input_shape:tuple,in_channels:int,out_channels:int,conv_kernel_size:int,conv_stride:int,conv_padding:int,
                 bias:bool,v_threshold:float=1.0,v_reset:float=0.0,tau:float=5,alpha:float=0.1,beta:float=0.5,
        ...
        super(Conv2dNoPoolingBlock,self).__init__(norm,input_shape,in_channels,out_channels,conv_kernel_size,conv_stride,conv_padding,
                                                  2,2,0,bias,v_threshold,v_reset,tau,alpha,beta,surrogate_type,surrogate_param)
...
class ResidualBlock(BaseBlock):
    def __init__(self,norm:str,input_shape:tuple,in_channels:int,out_channels:int,conv_kernel_size:tuple,conv_stride:tuple,conv_padding:tuple,
                 pool:bool,downsample:bool,bias:bool,v_threshold:float=1.0,v_reset:float=0.0,tau:float=5,alpha:float=0.1,beta:float=0.5,
                 surrogate_type:str='sigmoid',surrogate_param:float=2.0) -> None:
        ...
        self.lif_1=LIFNode(v_threshold=v_threshold,v_reset=v_reset,tau=tau,alpha=alpha,beta=beta,surrogate_type=surrogate_type,surrogate_param=surrogate_param)
        self.lif_2=LIFNode(v_threshold=v_threshold,v_reset=v_reset,tau=tau,alpha=alpha,beta=beta,surrogate_type=surrogate_type,surrogate_param=surrogate_param)
...
class SEWResidualBlock(ResidualBlock):
    def __init__(self,connect_function:str,norm:str,input_shape:tuple,in_channels:int,out_channels:int,conv_kernel_size:tuple,conv_stride:tuple,
        ...
        super(SEWResidualBlock,self).__init__(norm,input_shape,in_channels,out_channels,conv_kernel_size,conv_stride,conv_padding,pool,
                                              downsample,bias,v_threshold,v_reset,tau,alpha,beta,surrogate_type,surrogate_param)
        ...
        if self.downsample and self.norm=='BN':
            self.downsample_layer=nn.Sequential(
                ...
                LIFNode(v_threshold=v_threshold,v_reset=v_reset,tau=tau,alpha=alpha,beta=beta,surrogate_type=surrogate_type,surrogate_param=surrogate_param)
            )
        elif self.downsample and self.norm!='BN':
            self.downsample_layer=nn.Sequential(
                SeqToANNContainer(
